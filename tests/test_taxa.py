...
# def test_config():
#     builder = taxa_old.TaxonCacheBuilder(config)
...
#def test_get_undescribed():
 #   tracking_df = pd.read_csv("taxonomy/tracking_lists/all_tracked_species.csv")\


# def test_load_tracking_list_success():
#     tracking_file = "taxonomy/tracking_lists/all_tracked_species.csv"

#     tracking_df: pd.DataFrame = taxa_old.load_tracking_list(tracking_file)
...
#         tracking_df = taxa_old.load_tracking_list("not_a_file.csv")
    
#     # File without correct columns
#     with pytest.raises(KeyError):
#         tracking_df = taxa_old.load_tracking_list("tests/bad.csv")
...
#     mappings_df = taxa_old.load_mapping_list(map_file)
#     assert len(mappings_df) == 86
#     assert [col in mappings_df.columns for col in cols]
    
#     mappings_df = taxa_old.load_mapping_list("not_a_file")
#     assert len(mappings_df) == 0
#     assert [col in mappings_df.columns for col in cols]

#     mappings_df = taxa_old.load_mapping_list()
#     assert len(mappings_df) == 0
#     assert [col in mappings_df.columns for col in cols]


# def test_get_to_match_list():
#     tracking_df = taxa_old.load_tracking_list(tracking_file)
#     mapping_df = taxa_old.load_mapping_list(mapping_file)
#     to_match = taxa_old.get_to_match_list(tracking_df, mapping_df)
...
# def test_setup_dfs():
#     tracking_df, mapping_df, overrides_df = taxa_old.setup_dfs(
...
#     )
#     to_match = taxa_old.get_to_match_list(tracking_df, mapping_df)
#     to_match["sname_clean"] = taxa_old.get_name_overrides(to_match, overrides_df)
